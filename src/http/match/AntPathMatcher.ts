...
import quotemeta from "xregexp-quotemeta";
...
export default class AntPathMatcher implements PathMatcher {
    ...
    private cachePatterns: boolean = false;
...
class AntPathStringMatcher {
    ...
    constructor(pattern: string, caseSensitive: boolean = true) {
        ...
        this.pattern = XRegExp(regExpSource, caseSensitive ? 'g' : 'ig');
    }
    ...
    public matchStrings = (str: string, uriTemplateVariables?: Map<String, String>) => {
        ...
        const xRegExp = XRegExp(`^${this.pattern.source}$`, 'g');
    ...
    private patternQuote = (text: string) => {
        let slashEIndex = text.indexOf("\\E");
        if (slashEIndex == -1) {
            return `\\Q${text}\\E`;
        }
        let sb = [];
        ...
        while ((slashEIndex = text.indexOf("\\E", current)) != -1) {
            sb.push(text.substring(current, slashEIndex));
            current = slashEIndex + 2;
            sb.push("\\E\\\\E\\Q");
        }
        sb.push(text.substring(current, text.length));
        sb.push("\\E");
        return sb.join("");
    }
}
